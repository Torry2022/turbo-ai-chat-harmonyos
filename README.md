...
- 手机底部导航与宽窗口侧栏加入图标，宽侧栏保留副标题；手机导航采用随主题切换的毛玻璃背景，并调整底部内容避让。
- 修复重复导入时短名称丢失关键标识的问题；扫描目录不再导致整页按钮闪动，没有新增模型时保留当前选择和参数编辑。

**本地模型导入优化（v1.11.1）**

- 支持在设备上选择文件夹导入模型，保留 ZIP 导入与已推送目录扫描。
- 增加导入空间检查和分阶段错误提示，优化自动短名称并保留已有名称。

**PC 菜单与聊天交互（v1.11.0）**

- PC 标题栏新增对话、设置和帮助菜单，支持悬停切换菜单，并提供快捷键说明与关于窗口。
- 聊天输入框上方常驻当前模型名称和状态灯；未加载模型时点击输入区域，可前往模型页加载。
- 支持选择部分消息文本复制，PC 右键优先复制所选文本；优化发送与麦克风按钮切换及启动画面。
- 模型广场无匹配结果时显示图标提示，并可一键清空筛选；模型默认生成参数的来源说明见[参数说明](model-catalog/generation-defaults.md)。
...
- 模型默认生成参数来自仓库配置或 App 预设，并非逐模型调优结果；刷新目录不会自动覆盖内置预设和已安装模型的配置快照。维护规则见[生成参数说明](model-catalog/generation-defaults.md)。
...
- PC 窗口标题栏提供“对话、设置、帮助”菜单，可新建对话、打开外观设置，以及查看快捷键说明和应用版本。展开后，鼠标移到其他菜单项即可切换，按 Esc 关闭；手机和平板保留原有入口。
- 聊天运行状态条仅占页面内容区，不改变侧栏位置；宽屏聊天区与其他页面采用相同的最大宽度，单条消息另行限宽以便阅读。
- 输入区常驻显示模型名称与加载状态；未加载时点击输入区会前往模型页。手机和平板长按消息气泡、PC 右键点击气泡可打开复制菜单。
- 消息文本支持局部选取复制：触屏长按文字后调整选区，PC 使用鼠标拖选。选区限于同一个文本块，暂不支持跨 Markdown 段落、代码块或表格单元格连续选择。
...
- 思考块：分开展示思考过程与最终回答，也支持聊天模板已带起始 `<think>`、生成输出只带结束标签的模型；不改变模型默认的思考开关。思考内容占用输出预算，达到上限不等于模型无法运行。
...
- 导入本地 MNN 模型：支持选择文件夹（设备支持时）、zip 导入及 hdc 推送目录后扫描注册；正式 MNN 导出目录优先使用 `llm_config.json` 中的 `jinja.chat_template`
...
手机底栏和窄侧栏的入口显示为“API 服务”；宽侧栏显示“服务”，并保留“OpenAI API”副标题。
...
推荐优先使用 App 内模型广场安装模型；本地转换的模型可以选择文件夹或 zip 导入，手动推送目录作为调试备用。

- **模型广场**：在模型页打开模型广场，可刷新在线目录并从 ModelScope 下载预置或扩展模型；网络失败时继续使用最近一次有效缓存或安装包内置目录。
- **选择文件夹导入**：通过系统选择器选择一个完整 MNN 模型目录。App 会复制文件到沙箱，校验后注册，不修改原目录；若无法打开选择器，可重试或改用压缩包导入。
- **导入 zip**：选择结构完整的 MNN 模型目录压缩包。导入前检查暂存空间，解压仍需额外空间；失败提示区分复制、解压、校验与保存阶段。没有人为设置 2GB 大小上限，但不同系统版本的实际支持情况需分别验证。
- **手动推送目录**：适合大模型、离线调试或 zip 导入失败时使用，推送后在模型页扫描注册。

### 手动推送模型目录

App 内通过模型广场下载、选择文件夹或 zip 导入是推荐方式。以下 `hdc` 推送仅作调试备用。
...
无论通过文件夹、zip 还是目录扫描导入，App 都会读取 `llm_config.json` 中的 `is_visual` 自动识别图片能力。自动短名称兼容连字符、下划线和空格分隔，尽量保留模型代数、参数量、`VL` 等变体及量化标记；原名没有参数量时不会猜测。已有非空短名称不会自动覆盖；如需重新生成，可在导入模型配置中清空短名称后保存。
